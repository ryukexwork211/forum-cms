...
if(!function_exists('bbp_voting_hook_setting')) {
    function bbp_voting_hook_setting($value) {
        ...
        if($type == 'bool') {
            ...
            if(!empty($setting_value)) return $setting_value;
            return $value;
        }
    }
}
...
if(!function_exists('bbp_voting_field')) {
	function bbp_voting_field( $key, $name, $label = '', $description = '', $type = 'bool', $pro = false ) {
		...
		<tr valign="top">
			...
			<td>
				<?php if($type == 'bool') { ?>
					<input type="checkbox" <?php echo $attributes; ?> value="true" <?php if(get_option($key) == 'true') echo 'checked'; ?> />
				<?php } elseif(is_array($type)) { ?>
					<select <?php echo $attributes; ?>>
						<?php foreach($type as $option) { ?>
							<option value="<?php echo $option; ?>"<?php if(get_option($key) == $option) echo ' selected'; ?>><?php echo ucwords(str_replace('-', ' ', $option)); ?></option>
						<?php } ?>
					</select>
				<?php } else { ?>
